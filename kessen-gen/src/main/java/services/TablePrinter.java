...
import static services.DataReader.readUntilOffset;
import static services.DataReader.readUntil;
...
public class TablePrinter {
    ...
    public String getJapaneseFromBytes(byte[] data, Dictionnary japanese) {
        ...
        for (byte b:data) {
            if (b==MODE_F0_BYTE) {
            ...
            else if (b==MODE_BF_BYTE) {
                previousMode = currentMode;
                currentMode = CharType.MODE_BF;
            } else {
                ...
                else if (currentMode == CharType.MODE_BF) {
                    if (fdBytes==null) {
                        fdBytes = new byte[2];
                        fdBytes[0] = b;
                    } else {
                        fdBytes[1] = b;
                        res+= "{"+toHexString(MODE_BF_BYTE)+" "+bytesToHex(fdBytes).trim()+"}";
                        currentMode = previousMode;
                        fdBytes=null;
                    }
                }
